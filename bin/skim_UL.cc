...
int ScanChain(std::vector<std::string> const& inputfnames, std::string output_fname, std::string proc, std::string str_year, float scale_factor = 1){
  ...
  ElectronHandler muonHandler;
  JetMETHandler jetHandler;
  //ParticleDisambiguator particleDisambiguator;

  curdir->cd();

  BaseTree* tin = new BaseTree(inputfnames, { "Events" }, "");
  curdir->cd();
...
#undef SIMPLE_DATA_DIRECTIVE
  ...
  int nEventsTotal = tin->getNEvents();
  for (int ev=0; ev<nEventsTotal; ev++){
    if (SampleHelpers::doSignalInterrupt==1) break;

    clear_branches();
    tin->getEntry(ev);
    HelperFunctions::progressbar(ev, nEventsTotal);

    // Reconstruct all muons
    muonHandler.constructMuons();
    auto const& muons = muonHandler.getProducts();

    // Reconstruct all electrons
    electronHandler.constructElectrons();
    auto const& electrons = electronHandler.getProducts();

    // Reconstruct all jet and MET objects
    jetHandler.constructJetMET(&muons, &electrons);

    //particleDisambiguator.disambiguateParticles(muonHandler, electronHandler);
...
int main(int argc, char** argv){
  ...
  SampleHelpers::setDataPeriod(str_year);
