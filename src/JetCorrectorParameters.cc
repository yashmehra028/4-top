...
int JetCorrectorParameters::binIndex(const std::vector<float>& fX) const
{
  ...
  // for N=1, binary search overhead is too costly
  if (N == 2) {
    auto it = std::lower_bound(mRecords.begin(), mRecords.end(), fX);
    result = std::distance(mRecords.begin(), it);
    // lower bound returns the first element that is greater than the target, so subtract 1 to get target bin
    // unless we somehow underflow, but that should never happen unless jet eta < ~6
...
JetCorrectorParameters const & JetCorrectorParametersCollection::operator[](key_type k) const {
  ...
  JetCorrectorParameters const* res = nullptr;
  for (; i != iend; ++i) {
    if (k == i->first) res = &(i->second);
  }
  //std::cout << " cannot find key " << static_cast<int>(k) << std::endl;
  //assert(0);
  // if we got this far, we didn't have a proper index
  if (!res) throw std::out_of_range(Form("[JetCorrectorParameterCollecion::operator[]]: cannot find key %i", static_cast<int>(k)));
  return *res;
}
