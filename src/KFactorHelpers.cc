...
namespace KFactorHelpers{
  ...
  float getBeamEnergy(int const& year){
    ...
    default:
      IVYerr << "KFactorHelpers::getBeamEnergy: Beam energy for year " << year << " is not implemented." << endl;
      assert(0);
      break;
    }
    return beamEnergy;
  ...
  void getVVTopology(
    ...
    if (!doZZ && !doWZ && !doWW){
      IVYerr << "KFactorHelpers::getVVTopology: Invalid VV topology flag " << type << endl;
      assert(0);
      return;
    }
    ...
    std::vector<GenParticleObject*> lepMinusPlus[3][2]; // l-, l+
    std::vector<GenParticleObject*> lepNuNubar[3][2]; // nu, nub
    std::vector<GenParticleObject*> quarkAntiquark[6][2]; // q, qb
    for (auto it_part = genparticles.cbegin(); it_part != genparticles.cend(); it_part++){
      GenParticleObject* const& part = *it_part;
    ...
    */
    ...
    std::vector< std::pair<GenParticleObject*, GenParticleObject*> > tmpVhandle;
    ...
    std::pair<GenParticleObject*, GenParticleObject*> const* bestV1pair=nullptr;
    std::pair<GenParticleObject*, GenParticleObject*> const* bestV2pair=nullptr;
    ...
    if (!bestV1pair){ IVYerr << "KFactorHelpers::getVVTopology: Best V1 cannot be determined!" << endl; assert(0); }
    if (!bestV2pair){ IVYerr << "KFactorHelpers::getVVTopology: Best V2 cannot be determined!" << endl; assert(0); }
  ...
  double xsecRatio_qqZZ4f_QCD_NLO_NNLO_13TeV_byMass(double mass, bool is_4f_2f2fp, unsigned char order){ // is_4f_2f2fp: 0=4l, 1=2l2l'; order: 0=NLO/LO, 1=NNLO/NLO
    ...
    if (static_cast<size_t>(order+2)>=it_xsec->size()){ IVYerr << "KFactorHelpers::xsecRatio_qqZZ4f_QCD_NLO_NNLO_13TeV_byMass: Order " << order << " is not implemented." << endl; assert(0); }
  ...
  double xsecRatio_qqWW4f_QCD_NLO_NNLO_13TeV_byPt(double pt, unsigned char order){ // order: 0=NLO/LO, 1=NNLO/NLO
    ...
    if (order == 0){
      ...
      IVYerr << "KFactorHelpers::xsecRatio_qqWW4f_QCD_NLO_NNLO_13TeV_byPt: Order " << order << " is not implemented." << endl;
      assert(0);
      return 1;
    }
  }
  ...
  KFactorHandler_QCD_ggVV_Sig::KFactorHandler_QCD_ggVV_Sig(int const& year) :
    ...
    kfFile_NLO(nullptr)
  {
    TString strKFactorDir = ANALYSISPKGDATAPATH + "Kfactors/";
    ...
    case 2018:
      ...
      break;
    default:
      IVYerr << "KFactorHandler_QCD_ggVV_Sig::KFactorHandler_QCD_ggVV_Sig: Year " << year << " is not implemented." << endl;
      assert(0);
      break;
    }
  ...
  void KFactorHandler_QCD_ggVV_Sig::setup(){
    ...
    for (size_t ikf=0; ikf<sp_NNLO.size(); ikf++){
      if (!sp_NNLO.at(ikf)){ IVYerr << Form("KFactorHandler_QCD_ggVV_Sig::setup: NNLO K factor at location %lu cannot be found.", ikf) << endl; assert(0); }
    }
    ...
    for (size_t ikf=0; ikf<sp_NLO.size(); ikf++){
      if (!sp_NLO.at(ikf)){ IVYerr << Form("KFactorHandler_QCD_ggVV_Sig::setup: NLO K factor at location %lu cannot be found.", ikf) << endl; assert(0); }
    }
  ...
  void KFactorHandler_QCD_ggVV_Sig::eval(KFactorHelpers::KFactorType type, KFactorHelpers::KFactorType denominator, std::unordered_map<TString, float>& kfactors_map) const{
    ...
    if (it_arg == kfactors_map.end()){ IVYerr << Form("KFactorHandler_QCD_ggVV_Sig::eval: K factor evaluation argument, candidate mass with name %s, cannot be found.", KFactorHandler_QCD_ggVV_Sig::KFactorArgName.data()) << endl; assert(0); }
    ...
    fcn_Kfactor_QCD_qqWW(nullptr)
  {
    ...
    case 2011:
      sqrts=7;
      IVYerr << "KFactorHandler_EW_qqVV_Bkg::KFactorHandler_EW_qqVV_Bkg: sqrts for year " << year << " does not have QCD K factor functions implemented." << endl;
      assert(0);
      break;
    case 2012:
      sqrts=8;
    ...
    case 2018:
      ...
      break;
    default:
      IVYerr << "KFactorHandler_EW_qqVV_Bkg::KFactorHandler_EW_qqVV_Bkg: sqrts for year " << year << " is not implemented." << endl;
      assert(0);
      break;
    }
    ...
    else{ IVYerr << "KFactorHandler_EW_qqVV_Bkg::KFactorHandler_EW_qqVV_Bkg: Correction type " << type << " is not implemented." << endl; assert(0); }
  ...
  void KFactorHandler_EW_qqVV_Bkg::readTableFromFile(TString const& fname, std::vector< std::vector<double> >& table){
    ...
    if (!HostHelpers::FileReadable(fname.Data())){ IVYerr << "KFactorHandler_EW_qqVV_Bkg::readTableFromFile " << fname << " does not exist!" << endl; assert(0); }
  ...
  void KFactorHandler_EW_qqVV_Bkg::eval(
    ...
    if (!(bestV1pair.first && bestV1pair.second)){ IVYerr << "KFactorHandler_EW_qqVV_Bkg::eval: V1 is invalid." << endl; assert(0); }
    if (!(bestV2pair.first && bestV2pair.second)){ IVYerr << "KFactorHandler_EW_qqVV_Bkg::eval: V2 is invalid." << endl; assert(0); }
    ...
    fcn_Kfactor_QCD_qqWW(nullptr)
  {
    ...
    case 2018:
      ...
      break;
    default:
      IVYerr << "KFactorHandler_QCD_qqVV_Bkg::KFactorHandler_QCD_qqVV_Bkg: Year " << year << " is not implemented." << endl;
      assert(0);
      break;
    }
  }
  ...
  void KFactorHandler_QCD_qqVV_Bkg::eval(
    ...
    if (!(bestV1pair.first && bestV1pair.second)){ IVYerr << "KFactorHandler_QCD_qqVV_Bkg::eval: V1 is invalid." << endl; assert(0); }
    if (!(bestV2pair.first && bestV2pair.second)){ IVYerr << "KFactorHandler_QCD_qqVV_Bkg::eval: V2 is invalid." << endl; assert(0); }
