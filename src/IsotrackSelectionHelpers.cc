...
float IsotrackSelectionHelpers::relMiniIsoCharged(IsotrackObject const& part){ return part.extras.miniPFRelIso_chg; }
float IsotrackSelectionHelpers::absMiniIsoCharged(IsotrackObject const& part){ float pt = part.pt(); return relMiniIsoCharged(part)*pt; }

float IsotrackSelectionHelpers::relPFIsoCharged_DR0p3(IsotrackObject const& part){ return part.extras.pfRelIso03_chg; }
float IsotrackSelectionHelpers::absPFIsoCharged_DR0p3(IsotrackObject const& part){ float pt = part.pt(); return relPFIsoCharged_DR0p3(part)*pt; }

float IsotrackSelectionHelpers::relMiniIsoComb(IsotrackObject const& part){ return part.extras.miniPFRelIso_all; }
float IsotrackSelectionHelpers::absMiniIsoComb(IsotrackObject const& part){ float pt = part.pt(); return relMiniIsoComb(part)*pt; }

float IsotrackSelectionHelpers::relPFIsoComb_DR0p3(IsotrackObject const& part){ return part.extras.pfRelIso03_all; }
float IsotrackSelectionHelpers::absPFIsoComb_DR0p3(IsotrackObject const& part){ float pt = part.pt(); return relPFIsoComb_DR0p3(part)*pt; }
...
bool IsotrackSelectionHelpers::testVetoId(IsotrackObject const& part){
  ...
  unsigned int const abs_id = std::abs(part.pdgId());
  bool const isMuon = abs_id==13;
  bool const isElectron = abs_id==11;
  bool const isChargedHadron = abs_id>100;
  return ((isMuon || isChargedHadron || isElectron) && extras.fromPV && extras.isPFcand && std::abs(extras.dz)<0.1);
}
